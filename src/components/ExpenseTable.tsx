  ...
  return (
    <section className="overflow-hidden rounded-[1.75rem] border border-[var(--color-line)] bg-[var(--color-surface)] shadow-[var(--shadow-diffuse)]">
      <div className="flex items-center justify-between gap-4 border-b border-[var(--color-line)] px-6 py-5">
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-[var(--color-ink)]">Daily money log</h2>
          <p className="text-sm text-[var(--color-ink-soft)]">Income in, expenses out — every line kept for history.</p>
        </div>
        <button
          onClick={addRow}
      ...
      <div className="overflow-x-auto">
              ...
              {transactions.map((row) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <input type="checkbox" checked={row.cleared} onChange={(e) => updateRow(row.id, { cleared: e.target.checked })} className="h-4 w-4 cursor-pointer accent-[var(--color-accent)]" aria-label={`Mark ${row.name} cleared`} />
                    </td>
                    <td className="px-6 py-3 text-right">
                      <button onClick={() => deleteRow(row.id)} className="rounded-lg p-2 text-[var(--color-ink-soft)] opacity-0 transition hover:bg-[oklch(94%_0.04_25)] hover:text-[oklch(50%_0.18_25)] focus:opacity-100 group-hover:opacity-100 focus:outline-none focus-visible:shadow-[var(--ring-accent)]" aria-label="Delete entry">
                        <Trash size={16} />
                      </button>
                    </td>
                  </motion.tr>
        ...
        {transactions.length === 0 && (
          <div className="px-6 py-12 text-center text-sm text-[var(--color-ink-soft)]">No entries yet. Hit “Add entry” to start logging.</div>
        )}
      </div>
    </section>
  );
